...
class CinnLaunchOp : public framework::OperatorBase {
 ...
 private:
  void RunImpl(const framework::Scope& scope,
    ...
    auto input_tensors = details::GetTensors(scope, Inputs(kX));
    // ::cinn::common::Target
    ...
    std::shared_ptr<CinnScope> compiled_scope;
    ...
    CheckTensorEquivalent(input_tensors, input_compiled_tensors);
    details::AppendExecutionArguments(input_tensors, paddle2cinn_varmap,
                                      name2argument, &hold_buffers);

    // prepare output variables
    auto output_tensors = details::GetTensors(scope, Outputs(kOutputs));
    ...
    details::AppendExecutionArguments(output_tensors, paddle2cinn_varmap,
                                      name2argument, &hold_buffers);
    ...
    if (!temp_variables.empty()) {
      details::InitializeTempVar(temp_variable_names, compiled_scope, place,
                                 temp_scope);
      auto temp_tensors = details::GetTensors(*temp_scope, temp_variable_names);
      details::AppendExecutionArguments(temp_tensors, paddle2cinn_varmap,
                                        name2argument, &hold_buffers);
    }
